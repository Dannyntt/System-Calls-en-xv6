...
found:
  p->pid = allocpid();
  p->state = USED;
  p->traceid = 0; // no syscall being traced by default
...
kfork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // el hijo hereda la syscall que el padre estuviera monitoreando con trace
...
// Cuenta cuantos procesos de la tabla estan actualmente en estado RUNNABLE.
// Usado por sys_sysinfo().
int
countrunnable(void)
{
  struct proc *p;
